...
function Header() {
  ...
  return (
    ...
    <div className='header'>
      ...
      <p>Deliver to <span>Pakistan</span></p>
      </div>

      <div className='inputs'>
        <span className='items'>All <i className="fa-solid fa-caret-down"></i></span>
        <input type="text" placeholder='Search Amazon' />
        <span className='search'><i className="fa-solid fa-magnifying-glass"></i></span>
      </div>

      <div class="flex items-center md:order-2 space-x-1 md:space-x-0 rtl:space-x-reverse">
      <button type="button" data-dropdown-toggle="language-dropdown-menu" style={{fontSize: "20px",fontWeight: "700"}} class="inline-flex items-center font-medium justify-center px-4 py-2 text-sm= text-gray-900 dark:text-white rounded-lg cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 dark:hover:text-white">
        ...
        EN <i style={{paddingLeft: "4px", color: "gray"}} className="fa-solid fa-caret-down"></i>
      </button>
      ...
      <div className='signs'>
        <span className='top-sign'>Hello, sign in</span>
        <span className='down-sign'>Account & Lists<i className="fa-solid fa-caret-down"></i></span>
        <div className='dropdown'>
        <div className='sign-drop'>
          { isAuthenticated ? <button onClick={() => logout({ logoutParams: { returnTo: window.location.origin } })}>
      Log Out
    </button> : <button><span onClick={() => loginWithRedirect()}>Sign in</span></button>
          }
          
          <p>New cuntomer? <span>Start here.</span></p>
        </div>
